...
@section('pageTitle', 'زيادة الأرباح')
...
@section('content')
    <div class="inner-body user_information">
        @for ($i = 1; $i <= 3; $i++)
            @php
                $item = $items[$i] ?? null;
                $formId = 'increaseProfitsForm' . $i;
            @endphp
            <div class="mb-5">
                <h3 class="mb-3">العنصر {{ $i == 1 ? 'الأول' : ($i == 2 ? 'الثاني' : 'الثالث') }}</h3>

                <form id="{{ $formId }}" action="{{ route('dashboard.site-text.update', 'increase_profits') }}" method="POST"
                    enctype="multipart/form-data" autocomplete="off">
                    @csrf
                    @method('POST')
                    <input type="hidden" name="type" value="increase_profits">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 mt-3">
                            <div class="div_input_label">
                                <label class="label_style" for="description{{ $i }}">النسبة المئوية</label>
                                <input type="text" id="description{{ $i }}" name="description" class="form-control"
                                    value="{{ old('description', $item->description ?? '') }}" placeholder="مثال: +45%">
                                <small class="form-text text-muted">أدخل النسبة المئوية مثل: +45%</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <button type="submit" class="save_informations" id="submitBtn{{ $i }}">
                        ...
                        <span class="button-text">حفظ العنصر
                            {{ $i == 1 ? 'الأول' : ($i == 2 ? 'الثاني' : 'الثالث') }}</span>
                    </button>
                </form>
            </div>
        @endfor
        ...
        $(document).ready(function() {
            @for ($i = 1; $i <= 3; $i++)
                ...
                if (typeof handleFormSubmission === 'function') {
                    handleFormSubmission('#increaseProfitsForm{{ $i }}', {
                        successMessage: 'تم حفظ العنصر {{ $i == 1 ? 'الأول' : ($i == 2 ? 'الثاني' : 'الثالث') }} بنجاح',
                        errorMessage: 'حدث خطأ أثناء حفظ البيانات',
                        afterSubmit: function(response) {
                            $('#increaseProfitsForm{{ $i }}').find('.is-invalid').removeClass(
                                'is-invalid');
                            $('#increaseProfitsForm{{ $i }}').find('.error-container').remove();
