...
@section('pageTitle', 'تعديل سؤال شائع')

@section('content')
    <div class="inner-body user_information">
        <form id="updateFaqForm" action="{{ route('dashboard.faqs.update', $faq) }}" method="POST" autocomplete="off">
            @csrf
            @method('PUT')
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 mt-3">
                    <div class="div_input_label">
                        ...
                        <textarea id="answer"
                            ...
                            rows="6"
                            placeholder="قم بتحديث الإجابة">{{ old('answer', $faq->answer) }}</textarea>
                        @error('answer')
                            <small class="text-danger d-block mt-1">{{ $message }}</small>
                        @enderror
                    </div>
    ...
    <script>
        $(document).ready(function() {
            if (typeof handleFormSubmission === 'function') {
                handleFormSubmission('#updateFaqForm', {
                    successMessage: 'تم تحديث السؤال بنجاح',
                    errorMessage: 'حدث خطأ أثناء تحديث السؤال',
