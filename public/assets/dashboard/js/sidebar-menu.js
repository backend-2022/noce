...
// الاسكربتاية المعدلة بدل اللي فوقها علي طول
    ...
    // Check if server-side (PHP) has already set active classes
    // If so, skip JavaScript active class logic to avoid conflicts
    var hasServerActive = $(".sidebar-wrapper nav").find("a.active, li.active").length > 0;

    if (!hasServerActive) {
        // Only run JavaScript active class logic if PHP hasn't set any active classes
        ...
        $(".sidebar-wrapper nav ul>li a").filter(function () {
            ...
            if (link) {
                // Normalize link path (remove trailing slash, handle full URLs)
                var linkPath = link;
                try {
                ...
                // Match exact path only (not prefix matching to avoid false positives)
                // For dashboard home, only match if current is exactly /dashboard or /dashboard/
                if (current === linkPath) {
...
// استهدف الاتنين
$toggle_nav_top = $('.sidebar-close, .menu_bttn');
...
if ($('#pageWrapper').hasClass('compact-wrapper')) {
    // Check if server-side (PHP) has already set active classes
    // If so, skip JavaScript active class logic to avoid conflicts
    var hasServerActive = $(".sidebar-wrapper nav #sidebar-menu").find("a.active, li.active").length > 0;
    ...
    if (!hasServerActive) {
        ...
        $(".sidebar-wrapper nav #sidebar-menu ul .simplebar-mask li a, .sidebar-wrapper nav #sidebar-menu ul li a").filter(function () {
            var link = $(this).attr("href");
            if (link) {
                // Normalize link path (remove trailing slash, handle full URLs)
                var linkPath = link;
